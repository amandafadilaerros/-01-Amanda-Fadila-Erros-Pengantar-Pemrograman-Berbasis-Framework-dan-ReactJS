type FooterLinkProps = {
    text: string;
    url: string;
};

function FooterLink({ text, url}: FooterLinkProps){
    return <a className="transition hover:text-black" href={url}>{text}</a>
}

export default function Footer(){
    return(
        <footer className="pt-10 px-8 pb-16 bg-gradient-to-b bg-blue-800">
            <div className="flex justify-between gap-6">
                <div className="flex gap-6 text-sm font-medium text-zinc-200 dark:text-zinc-200">
                    <FooterLink text="Tentang Saya" url="/" />
                    <FooterLink text="Projects" url="/projects" />
                    <FooterLink text="Essays" url="/essays" />
                </div>
                <p className="text-sm text-zinc-400">©2025 Amanda Fadila Erros. All right reserved.</p>
            </div>
        </footer>
    );
}